...
const generatorSource = readFileSync(join(root, "scripts/authorForestPetSprites.mjs"), "utf8");
...
const centroid = (rows, keys) => {
  const points = [];
  rows.forEach((row, y) => [...row].forEach((key, x) => keys.has(key) && points.push([x, y])));
...
for (const sprite of sprites) {
  ...
  const lightCenter = centroid(sprite.rows, new Set(["l", "h", "q", "g", "k"]));
  const shadowCenter = centroid(sprite.rows, new Set(["s", "b", "d", "r"]));
  ...
  if (antiAliasPixels < 60) failures.push(`${sprite.id}: insufficient baked anti-alias edge coverage`);
  ...
  details.push({
    ...
    transitionPixels,
    unusedPaletteKeys,
    combinedLightCentroid: lightCenter,
    combinedShadowCentroid: shadowCenter,
...
if (sprites.length !== 10) failures.push(`expected 10 forest sprites, received ${sprites.length}`);
