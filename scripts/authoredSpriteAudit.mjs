...
const generatorSource = readFileSync(join(root, `scripts/author${element[0].toUpperCase()}${element.slice(1)}PetSprites.mjs`), "utf8");
const spritePattern = /  "([^"]+)": \{([\s\S]*?)\n    pixels: \[\n([\s\S]*?)\n    \]\n  \}/g;
const sprites = [...source.matchAll(spritePattern)].map((match) => {
  const [, id, header, pixelBlock] = match;
  return {
    id,
    palette: [...header.matchAll(/"([a-z])": \{ color: "([^"]+)", ramp: "([^"]+)", shade: "([^"]+)" \}/g)].map((entry) => ({
      key: entry[1],
      color: entry[2],
      ramp: entry[3],
      shade: entry[4]
...
for (const sprite of sprites) {
  ...
  if (!usedKeys.has("o") || !usedKeys.has("v")) failures.push(`${sprite.id}: missing variable outline roles`);
...
if (sprites.length !== 10) failures.push(`expected 10 ${element} sprites, received ${sprites.length}`);
if (highestMaskIou.value >= 0.72) failures.push(`highest silhouette IoU is too similar: ${highestMaskIou.pair.join(" / ")}`);
if (!/x1="0" y1="0" x2="1" y2="1"/.test(generatorSource) && !generatorSource.includes("petSpriteBake.mjs")) {
  failures.push("generator does not encode the shared left-top to right-bottom light direction");
}

console.log(JSON.stringify({ element, ok: failures.length === 0, sprites: sprites.length, highestMaskIou, details, failures }, null, 2));
